...
<style>
    ...
    .side-logo { width: 34px; height: 34px; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 14px rgba(6, 182, 212, 0.35); }
    .side-logo img { width: 34px; height: 34px; display: block; }
    ...
    section[data-testid="stSidebar"] .stButton > button {
        ...
        padding: 0.55rem 1rem !important; font-size: 0.82rem !important;
    ...
    .side-history-meta { flex: 1; min-width: 0; }
    .side-history-time { font-size: 0.68rem; color: #64748b; font-weight: 600; margin-bottom: 0.2rem; }
    .side-history-text {
    ...
    hr { border-color: #1c1f26; margin: 1.5rem 0; }
</style>
...
with st.sidebar:
    st.markdown(
        ...
        <div class="side-brand">
            <div class="side-logo"><img src="{LOGO_DATA_URI}" alt="PixelSage" /></div>
            <div class="side-brand-text">
                <h3>PixelSage</h3>
                <p>IMAGE ANALYSIS STUDIO</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    with tab_history:
        ...
        if not history:
            ...
            for idx, entry in enumerate(history):
                ...
                st.markdown(
                    f"""
                    <div class="side-history-card">
                        {thumb_html}
                        <div class="side-history-meta">
                            <div class="side-history-time">🕐 {entry['timestamp']}</div>
                            <div class="side-history-text">{entry['description']}</div>
                            <span class="side-history-tag">{entry['detail_level']}</span>
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True
                ...
                with st.expander("View / Delete", expanded=False):
                    st.markdown(
                        ...
                        f"line-height:1.6;'>{entry['description']}</div>",
                        unsafe_allow_html=True
                    )
                    st.code(entry['description'], language=None)
                    if st.button("🗑️ Delete", key=f"side_del_{idx}_{entry['id']}", use_container_width=True):
...
if uploaded_file:
    ...
    analyze_clicked = st.button("🔍  Analyze Image", use_container_width=True, key="main_analyze_btn")
